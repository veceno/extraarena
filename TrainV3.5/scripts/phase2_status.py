#!/usr/bin/env python3
"""Print a compact status line for a TrainV3 Phase 2 mixed-matrix run."""
...
DEFAULT_LATEST = ROOT / "TrainV3.5" / "runs" / "latest_phase2_run.txt"


def main(argv: list[str]) -> int:
    run_dir = _resolve_run_dir(argv[1] if len(argv) > 1 else None)
    metrics_path = run_dir / "metrics.jsonl"
    config_path = run_dir / "phase2_config.json"
...
def _resolve_run_dir(raw: str | None) -> Path:
    ...
    candidates = sorted((ROOT / "TrainV3.5" / "runs").glob("phase2_mixed_*"))
    if not candidates:
        raise SystemExit("no Phase 2 run found; pass run_dir explicitly")
