#!/usr/bin/env python3
"""Print a compact status line for a TrainV3 Phase 8 V4-max correction run."""
...
DEFAULT_LATEST = ROOT / "TrainV3.5" / "runs" / "latest_phase8_run.txt"


def main(argv: list[str]) -> int:
    run_dir = _resolve_run_dir(argv[1] if len(argv) > 1 else None)
    metrics_path = run_dir / "metrics.jsonl"
    config_path = run_dir / "phase8_config.json"
    ...
    if not updates:
        print(json.dumps({"run": str(run_dir), "update": 0, "target_updates": target, "metrics": str(metrics_path)}))
        return 0
    ...
    payload = {
        ...
        "adaptive_strength": _nested_float(last, "v5_mode", "adaptive_strength"),
        "enemy_known": bool(_nested(last, "v5_mode", "enemy_hand_known"))
        or bool(_nested(last, "v5_mode", "enemy_deck_known")),
        "draw_assist": bool(_nested(last, "v5_mode", "draw_assist_enabled")),
        "assist_profile_id": _nested_int(last, "assist_mode", "assist_profile_id"),
        "target_v4max_score": config.get("target_v4max_score"),
...
def _resolve_run_dir(raw: str | None) -> Path:
    ...
    candidates = sorted((ROOT / "TrainV3.5" / "runs").glob("phase8_v4max_correction_*"))
    if not candidates:
        raise SystemExit("no Phase 8 run found; pass run_dir explicitly")
...
def _nested_float(data: dict, *keys: str) -> float | None:
