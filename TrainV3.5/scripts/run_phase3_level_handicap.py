#!/usr/bin/env python3
"""Run Extra-LR V5 Phase 3 level-handicap hardening training."""
...
def main() -> int:
    run_name = _env_str("PHASE3_RUN_NAME", "phase3_level_handicap_noassist")
    env_count = _env_int("PHASE3_ENV_COUNT", 4096)
    steps_per_update = _env_int("PHASE3_STEPS_PER_UPDATE", 32)
    updates = _env_int("PHASE3_UPDATES", 1000)
    minibatch_size = _env_int("PHASE3_MINIBATCH_SIZE", 4096)
    checkpoint_every = _env_int("PHASE3_CHECKPOINT_EVERY", 100)
    hidden_dim = _env_int("PHASE3_HIDDEN_DIM", 256)
    action_hidden_dim = _env_int("PHASE3_ACTION_HIDDEN_DIM", 128)
    learning_rate = _env_float("PHASE3_LR", 0.00025)
    entropy_coef = _env_float("PHASE3_ENTROPY_COEF", 0.018)
    max_grad_norm = _env_optional_float("PHASE3_MAX_GRAD_NORM", 0.5)
    strengths = _env_float_tuple("PHASE3_ADAPTIVE_STRENGTHS", (0.25, 0.5, 0.75, 1.0))
    ...
    (out_root / "latest_phase3_run.txt").write_text(str(run_dir) + "\n", encoding="utf-8")
    ...
    config_snapshot = {
        ...
        "assist_policy": "no_assist",
        "private_info_policy": "enemy_hidden",
        "library_path": str(library_path),
    }
    (run_dir / "phase3_config.json").write_text(
        json.dumps(config_snapshot, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print("PHASE3_RUN_DIR", run_dir, flush=True)
    print("PHASE3_CONFIG", json.dumps(config_snapshot, sort_keys=True), flush=True)

    manifest = generate_v5_trace_pool(
        [
            V5TraceScenario(
                scenario_key="phase3_level_handicap_noassist",
                seeds=tuple(40_000 + idx * 23 for idx in range(trace_seed_count)),
                steps=12,
                ...
                draw_assist_modes=({"draw_assist_enabled": False, "draw_assist_strength": 0.0},),
                assist_modes=(
                    {
    ...
    train_config = RustPPOTrainingConfig(
        ...
        curriculum_metadata={
            "phase": "phase3_level_handicap_noassist",
            "machine": "macbook_pro_m4_pro_24gb",
            "assist_policy": "no_assist",
            "private_info_policy": "enemy_hidden",
            "teacher_hint_policy": "off",
            "level_handicap_policy": "oracle_python_trace_levels_rust_runtime",
            "level_modes": level_modes,
    ...
    print("PHASE3_RUN_RESULT", json.dumps(summary, sort_keys=True), flush=True)
    return 0


def _level_modes() -> list[dict[str, int | str]]:
    value = os.environ.get("PHASE3_LEVEL_MODES_JSON")
    if value is not None and value.strip():
        parsed = json.loads(value)
        if not isinstance(parsed, list) or not parsed:
            raise ValueError("PHASE3_LEVEL_MODES_JSON must be a non-empty JSON list")
        return [_normalize_level_mode(item) for item in parsed]
    return [
        {"p1_level": 1, "p2_level": 1, "label": "equal_l1"},
        {"p1_level": 1, "p2_level": 2, "label": "p1_l1_vs_p2_l2"},
        {"p1_level": 1, "p2_level": 3, "label": "p1_l1_vs_p2_l3"},
        {"p1_level": 2, "p2_level": 1, "label": "p1_l2_vs_p2_l1"},
        {"p1_level": 3, "p2_level": 1, "label": "p1_l3_vs_p2_l1"},
    ]
