#!/usr/bin/env python3
"""Play ONE battle as p1 LLM vs V4-Max. Seed 8002, llm_first."""
...
def llm_choose_action(state, legal, strategic_notes, battle_history):
    ...
    my_board_s = ", ".join(f"id={c.get('instance_id','')[:8]} atk={c.get('attack',0)} hp={c.get('hp_current',0)} {'READY' if c.get('can_attack') and c.get('is_ready') else 'SLEEP'}{' TAUNT' if 'taunt' in c.get('mechanics',[]) else ''}{' SHIELD' if 'shield' in c.get('mechanics',[]) else ''}{' CHARGE' if 'charge' in c.get('mechanics',[]) else ''}" for c in my_board) or "(empty)"
    opp_board_s = ", ".join(f"atk={c.get('attack',0)} hp={c.get('hp_current',0)} {'TAUNT' if 'taunt' in c.get('mechanics',[]) else ''}{' CHARGE' if 'charge' in c.get('mechanics',[]) else ''}" for c in opp_board) or "(empty)"
    my_hand_s = ", ".join(f"cost={c.get('mana_cost',0)} atk={c.get('attack',0)} hp={c.get('hp',0)}" for c in my_hand) or "(empty)"
...
LEGAL: {la_count}
Examples: play_card={json.dumps(la_examples.get('play_card',[])[:2])}, attack={json.dumps(la_examples.get('attack',[])[:2])}

STRATEGIC_NOTES (this battle so far):
{chr(10).join(strategic_notes[-3:]) if strategic_notes else "(start of battle)"}

RECENT MOVES:
{chr(10).join(battle_history[-5:]) if battle_history else "(start)"}
...
battle = {"seed":8002,"starting_player":"llm","agent_name":"mix-w2-llmfirst"}
strategic_notes = []
battle_history = []
