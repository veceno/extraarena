...
`rl` p1 the battles auto-play (model-vs-model, no L2 needed).
...
2. **Dispatch**: `start_series` per agent with varied `seed`/`starting_player`/
   `deck_strategy`. For `llm` p1, hand each `match_id` to a Level-2 agent.
...
3. Cross-check `degraded`: open `get_battle_group_manifest` and verify no
   `battles_results[].degraded`; or check `v5/meta.json bot_policy.weights_hash`
   == `sha256(<onnx>)[:16]` for the opponent (proves the real model played).
4. `download_battle_logs group_id=<gid> format=zip` → ship the archive path to
   the trainer (Level 0).
...
`v5` (reserved — implement + `register` to activate), `random`/`greedy_face`/
