...
// RLHF: экран превью колоды (только для случайной колоды) перед запуском отсчёта.
let arenaCardsMap = null;
let deckPreviewPromise = null;
let deckPreviewMatchId = null;
...
// ── RLHF: каталог карт + экран превью колоды перед боем ──
async function ensureArenaCardsCatalog() {
  if (arenaCardsMap) return arenaCardsMap;
  try {
    const r = await fetch('/api/cards');
    const d = await r.json();
    arenaCardsMap = new Map();
    for (const c of (d.cards || [])) arenaCardsMap.set(Number(c.id), c);
  } catch (e) {
    console.warn('[ARENA] /api/cards catalog load failed:', e);
    arenaCardsMap = new Map();
  }
  return arenaCardsMap;
}
function arenaCardById(id) {
  return (arenaCardsMap && arenaCardsMap.get(Number(id))) || null;
}
function buildDeckPreviewCell(id, isHero) {
  const card = arenaCardById(id) || {};
  const cell = document.createElement('div');
  cell.className = 'deck-preview-cell';
  cell.setAttribute('role', 'listitem');
  const img = document.createElement('img');
  img.alt = card.name || ('card ' + id);
  img.src = `/DesignAssets/CardsPreview/w384/${Number(id)}.webp`;
  img.onerror = () => {
    img.onerror = () => { img.src = '/DesignAssets/Cards/9.png'; };
    img.src = `/DesignAssets/Cards/${Number(id)}.png`;
  };
  cell.appendChild(img);
  const nm = document.createElement('div');
  nm.className = 'dp-name' + (isHero ? ' dp-hero' : '');
  nm.textContent = (isHero ? '★ ' : '') + (card.name || ('#' + id));
  cell.appendChild(nm);
  const st = document.createElement('div');
  st.className = 'dp-stats';
  const mana = card.mana_cost ?? card.mana ?? null;
  const atk = card.base_attack ?? card.attack ?? null;
  const hp = card.base_hp ?? card.hp ?? null;
  const parts = [];
  if (mana != null) parts.push('⭐' + mana);
  if (atk != null) parts.push('⚔️' + atk);
  if (hp != null) parts.push('❤️' + hp);
  st.textContent = parts.join(' ');
  cell.appendChild(st);
  return cell;
}
async function showDeckPreviewOverlay(cardIds) {
  try { await ensureArenaCardsCatalog(); } catch (_) {}
  const grid = document.getElementById('deck-preview-grid');
  if (grid) {
    grid.innerHTML = '';
    cardIds.forEach((id, idx) => grid.appendChild(buildDeckPreviewCell(id, idx === 0)));
  }
  const screen = document.getElementById('deck-preview-screen');
  if (screen) {
    screen.classList.remove('is-hidden');
    screen.setAttribute('aria-hidden', 'false');
  }
  await new Promise((resolve) => {
    const btn = document.getElementById('deck-preview-continue');
...
// Возвращает Promise, который резолвится, когда можно стартовать отсчёт:
// для случайной колоды — после клика «Продолжить»; для остальных — сразу.
// Гейт кешируется по match_id, чтобы каждый бой серии (включая 2..N) получал свой превью,
// а повторные state_changed в рамках одного боя не запускали отсчёт досрочно.
function maybeShowDeckPreview(state) {
  const mid = state && state.match_id;
  if (mid && mid !== deckPreviewMatchId) {
    deckPreviewMatchId = mid;
    deckPreviewPromise = null;
  }
  if (deckPreviewPromise) return deckPreviewPromise;
  const src = state && state.p1_deck_source;
  const isRandom = src && src.type === 'random';
  const cardIds = Array.isArray(state && state.p1_deck_card_ids) ? state.p1_deck_card_ids : [];
  if (!isRandom || !cardIds.length) {
    deckPreviewPromise = Promise.resolve();
    return deckPreviewPromise;
  }
  deckPreviewPromise = showDeckPreviewOverlay(cardIds);
  return deckPreviewPromise;
}
...
function handleStateChanged(eventData) {
  ...
  if (!prebattleComplete) {
    ...
    maybeShowDeckPreview(newState).then(() => startPrebattleSequence());
    return;
  }
...
async function loadBattleState() {
  try {
    ...
    if (!prebattleComplete) {
      maybeShowDeckPreview(state).then(() => startPrebattleSequence());
    } else {
      pendingInitialBattleState = null;
      renderBattleState(state);
    }
...
function handleGameOver(data) {
  ...
  // Сливаем поля серии (group_id/has_next_battle/...) из game_over-пейлоада в
  // currentState — на случай, если state_changed с ними уже не придёт. Это
  // гарантирует, что кнопка «Следующий бой» корректно знает о продолжении.
  // Поля могут быть как на верхнем уровне data (случай surrender через сокет —
  // _broadcast_game_over), так и внутри data.state (обычный game_over через
  // _broadcast) — уважаем оба источника.
  // srcState в области видимости функции (а не внутри if): при обычном game_over
  // winner_id лежит в data.state (payload _broadcast = {match_id, state:
  // get_full_state, data:{}}), на верхнем уровне data его НЕТ. Раньше цепочка
  // winner_id читала только data.winner_id и currentState?.winner_id, а
  // currentState.winner_id для human-kill выставлялся с задержкой (HTTP-ответ
  // гоняется с сокетным game_over) → null → «Ничья» при реальной победе.
  const srcState = (data && data.state) || {};
  if (data && currentState) {
    const seriesKeys = ['group_id', 'battle_index', 'battles_planned', 'has_next_battle'];
    for (const k of seriesKeys) {
      if (data[k] !== undefined) currentState[k] = data[k];
      else if (srcState[k] !== undefined) currentState[k] = srcState[k];
    }
  }
  ...
  // Извлекаем данные о победителе: srcState.winner_id (обычный game_over) —
  // основной источник; data.winner_id (surrender через _broadcast_game_over) —
  // верхний уровень; currentState — последний рельсовый резерв.
  const winnerId = data.winner_id ?? data.winner ?? srcState.winner_id ?? srcState.winner ?? currentState?.winner_id ?? null;
  const outcome = winnerId == null ? 'draw' : (String(winnerId) === String(userId) ? 'victory' : 'defeat');
...
// Следующий бой серии: POST /api/match/find {group_id} → редирект на новый бой.
// Сервер вернёт {status:'found', redirect_url} либо 404 {error:'series_complete'}.
// Без group_id (вне серии, напр. одиночный тестовый бой) — выход в меню среды.
async function doNextBattle() {
  const groupId = currentState && currentState.group_id;
  if (!groupId) {
    console.warn('[ARENA] doNextBattle: нет group_id — выход в меню среды');
  ...
  try {
    const resp = await fetch('/api/match/find', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ group_id: groupId }),
    });
    if (resp.status === 404) {
      console.log('[ARENA] Серия завершена (series_complete)');
      if (nextBtn) {
        nextBtn.disabled = true;
        nextBtn.textContent = 'Бои завершены';
      }
      return;
    }
    if (!resp.ok) {
      console.error('[ARENA] match/find error', resp.status);
      window.location.replace('/');
      return;
    }
    const data = await resp.json();
    if (data && data.redirect_url) {
      console.log('[ARENA] Переход к следующему бою:', data.redirect_url);
      window.location.replace(data.redirect_url);
    } else {
      window.location.replace('/');
    }
  } catch (err) {
    console.error('[ARENA] doNextBattle failed', err);
    window.location.replace('/');
  }
}
...
function showBattleResult(outcome, trophyDelta, trophyTotal, coinsDelta, coinsTotal, starsDelta, starsTotal) {
  ...
  // Настройка кнопок результата серии.
  // «Следующий бой» активна, если в серии есть следующий бой (has_next_battle);
  // иначе серая/недоступная «Бои завершены». «Завершить» — выход в меню среды:
  // пока серия продолжается, это мелкая вторичная «Завершить предварительно»;
  // когда бои завершены (next отключён) — становится основным действием «Завершить».
  const hasNext = !!(currentState && currentState.has_next_battle);
  const nextBtn = document.getElementById('result-next-btn');
  if (nextBtn) {
    nextBtn.disabled = !hasNext;
    nextBtn.textContent = hasNext ? 'Следующий бой' : 'Бои завершены';
    nextBtn.setAttribute('aria-disabled', hasNext ? 'false' : 'true');
  }
  const finishBtn = document.getElementById('result-finish-btn');
  if (finishBtn) {
    if (hasNext) {
      finishBtn.textContent = 'Завершить предварительно';
      finishBtn.className = 'btn result-btn-finish';
    } else {
      // Серия завершена — «Завершить» становится основной кнопкой.
      finishBtn.textContent = 'Завершить';
      finishBtn.className = 'btn primary result-btn-primary';
    }
...
function bindUIHandlers() {
  ...
  // Кнопка «Следующий бой»: продолжение серии (POST /api/match/find {group_id}).
  const resultNextBtn = document.getElementById('result-next-btn');
  if (resultNextBtn) {
    resultNextBtn.addEventListener('click', () => {
      console.log('[ARENA] Запрос следующего боя серии');
      doNextBattle();
    });
  }

  // Кнопка «Завершить» / «Завершить предварительно»: финализируем серию
  // на сервере (POST /api/groups/{gid}/finish — иначе статус серии зависает
  // в «running», т.к. manifest.finalize() без этого не вызывается), затем уходим
  // в главное меню RLHF-среды. sendBeacon — чтобы запрос долетел при навигации.
  const resultFinishBtn = document.getElementById('result-finish-btn');
  if (resultFinishBtn) {
    resultFinishBtn.addEventListener('click', () => {
      console.log('[ARENA] Досрочное завершение серии');
      finishSeriesAndLeave();
    });
  }
  ...
  if (resultOverlay) {
    resultOverlay.addEventListener('click', () => {
      console.log('[ARENA] Завершение серии (клик на overlay)');
      finishSeriesAndLeave();
    });
  }
}

// Финализация серии на сервере и выход в главное меню RLHF-среды.
// sendBeacon долетает даже при немедленной навигации; тело не нужно —
// хендлер читает только group_id из пути. Без этого серия, завершённая
// кнопкой/уходом, висела в статусе «running» (manifest.finalize не вызывался).
function finishSeriesAndLeave() {
  const groupId = currentState && currentState.group_id;
  if (groupId) {
    try {
      navigator.sendBeacon(`/api/groups/${encodeURIComponent(groupId)}/finish`);
    } catch (e) { /* fire-and-forget: навигация уйдёт в любом случае */ }
  }
  window.location.replace('/');
}
...
window.ExtraArenaAppBack = function() {
  ...
  // Кнопка «Сдача» удалена из арены RLHF-среды → аппаратный «Назад» не открывает
  // модал сдачи, а выходит в главное меню среды (как «Завершить»).
  finishSeriesAndLeave();
  return true;
};
